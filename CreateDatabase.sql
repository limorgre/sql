...
CREATE TABLE InsCategory --טבלת קטגוריות מוצרים
...
CREATE TABLE DiscountPackage  -- טבלת חבילות הנחה-לכל מוצר חבילת הנחה אחת לשימוש  
...
CREATE TABLE InsProducts --טבלת מוצרי ביטוח
(InsProductId INT CONSTRAINT InsProducts_InsProductId_pk PRIMARY KEY,
 ...
 DiscountPackageID INT CONSTRAINT InsProducts_DiscountPackageID_fk FOREIGN KEY REFERENCES DiscountPackage(DiscountPackageID),--חבילת הנחה כיום היא אחוז הנחה קבוע למספר שנים לכל מוצר
 "Indemnity or compensation" VARCHAR(2) NOT NULL, -- האם מוצר שיפוי או פיצוי
 StartDate DATE NOT NULL,
 EndDate DATE,
...
CREATE TABLE Marketers --טבלת משווקים
...
CREATE TABLE CreditCards --פרטי אשראי יכולים להיות של הלקוח או כל בן משפחה מקרבה ראשונה שאינו לקוח 
( CardNumber VARCHAR(20)  CONSTRAINT CreditCards_CardNumber_pk PRIMARY KEY,
 CardType VARCHAR(25) NOT NULL, 
 ExpMonth INT NOT NULL,
 ExpYear INT NOT NULL,
 Num_3 VARCHAR(5) NOT NULL,--ספרות בגב הכרטיס
 CreditOwnerId VARCHAR(9) NOT NULL,--ת.ז. בעל הכרטיס
...
CREATE TABLE Customers --טבלת לקוחות= פרטי מבוטח ראשי/משני
(Id VARCHAR(9) CONSTRAINT Customers_ID_pk PRIMARY KEY,--ת.ז. מבוטח
 Lname VARCHAR(15) NOT NULL,
 Fname VARCHAR(10) NOT NULL,
...
CREATE TABLE Direct_Debit --פרטי בנק של המבוטח הראשי/משני =לקוחות 
...
CREATE TABLE "Policy"  --טבלת פוליסות
(PolicyId VARCHAR(15) CONSTRAINT Policy_PolicyId_pk PRIMARY KEY,
 EmployeeID INT NOT NULL CONSTRAINT Policy_EmployeeID_fk FOREIGN KEY REFERENCES Marketers(EmployeeID) , --ת.ז. משווק
 ...
 CREATE TABLE PolicyDetails  --טבלת פרטי פוליסות
(PolicyId VARCHAR(15) CONSTRAINT PolicyDetails_PolicyId_fk FOREIGN KEY REFERENCES Policy(PolicyId), 
 InsuredID VARCHAR(9) CONSTRAINT PolicyDetails_InsuredID_fk FOREIGN KEY REFERENCES Customers(ID), -- (ת.ז. מבוטח (ראשי/משני/ילדים
 InsProductId INT CONSTRAINT PolicyDetails_InsProductId_fk FOREIGN KEY REFERENCES  InsProducts(InsProductId), --מספר מוצר ביטוח
 InsuredStatus INT NOT NULL, --  סטטוס המבוטח  1=ראשי  2=משני 3=ילדים
 Policy_Start_Date DATE NOT NULL,
 Policy_End_Date DATE NULL,
 Policy_End_Reason VARCHAR(15) Null, 
 Price_Before_Discuont1 MONEY NOT NULL,
 DiscountPackageID INT CONSTRAINT PolicyDetails_DiscountPackageID_fk FOREIGN KEY REFERENCES  DiscountPackage(DiscountPackageID),-- החלטת המשווק האם לתת הנחה לפוליסה או לא, לכן העמודה מופיעה בטבלה זו  
 CompensationSum MONEY NULL,  --סכום הפיצוי - בפוליסת פיצוי בלבד
 Smoke BIT NOT NULL ,  -- האם מעשן (ביום הפקת הפוליסה)  1=כן  0=לא
 CONSTRAINT PolicyDetails_PolicyId_InsuredIDֹֹֹ_InsProductId_pk PRIMARY KEY (PolicyId, InsuredID, InsProductId)
 )
